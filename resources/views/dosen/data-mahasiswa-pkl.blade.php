        ...
        <div class="flex flex-col xl:flex-row xl:items-center justify-between gap-4">
            <div>
                <h2 class="text-2xl font-black text-gray-900 dark:text-white tracking-tight flex items-center gap-3">
                    <div class="w-10 h-10 rounded-xl bg-purple-100 dark:bg-purple-500/10 text-purple-600 flex items-center justify-center shrink-0">
                        <i class="fa-solid fa-briefcase text-xl"></i>
                    </div>
                    Data Mahasiswa PKL Reguler
                </h2>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Daftar anak bimbing yang mendaftar jalur Praktik Kerja Lapangan standar perguruan tinggi.</p>
            </div>
            ...
            <div class="flex flex-col lg:flex-row gap-3 w-full xl:w-auto">
                <div class="bg-gray-100/80 dark:bg-surface-800 p-1.5 rounded-2xl flex flex-nowrap overflow-x-auto custom-scrollbar gap-1 shadow-sm border border-gray-200/50 dark:border-surface-700 max-w-full">
                    <button @click="tab = 'berjalan'" :class="tab === 'berjalan' ? 'bg-white dark:bg-surface-600 text-purple-600 dark:text-purple-400 shadow-sm' : 'text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200'" class="px-4 py-2 rounded-xl text-sm font-bold transition-all flex items-center gap-2 whitespace-nowrap">
                ...
                <div class="flex flex-col sm:flex-row gap-2">
                    <div class="relative flex-1 sm:flex-initial">
                        ...
                        <input type="text" x-model="search" placeholder="Cari nama atau NIM..." class="pl-10 pr-4 py-2 w-full sm:w-64 bg-white dark:bg-surface-800 border border-gray-200 dark:border-surface-700 rounded-xl text-sm focus:ring-purple-500 focus:border-purple-500 dark:text-white shadow-sm transition-all h-full outline-none">
                    ...
                    <a href="{{ route('dosen.pdf.pkl') }}" target="_blank" class="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-xl bg-purple-600 hover:bg-purple-700 text-white text-sm font-bold shadow-md shadow-purple-500/20 transition-all active:scale-95 whitespace-nowrap">
                ...
                <table class="w-full text-sm text-left">
                    <thead class="bg-gray-50 dark:bg-surface-900 text-xs text-gray-500 dark:text-gray-400 uppercase tracking-widest font-bold">
                        <tr>
                            ...
                            <th class="px-6 py-4">Riset Institusi</th>
                        ...
                        <tr x-show="search === '' || $el.dataset.search.includes(search.toLowerCase())" data-search="{{ strtolower(e($p->nama . ' ' . $p->nim . ' ' . $p->tempat_riset)) }}" class="hover:bg-gray-50/50 dark:hover:bg-surface-800/50 transition-colors group">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <div class="w-10 h-10 rounded-full bg-purple-50 dark:bg-purple-900/20 text-purple-600 flex items-center justify-center shrink-0"><i class="fa-solid fa-user-graduate"></i></div>
                            ...
                            <td class="px-6 py-4">
                                <div class="flex flex-wrap items-center justify-center gap-2">
                                    <span class="inline-flex items-center gap-1.5 px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider border {{ $p->lp ? 'bg-emerald-50 text-emerald-600 border-emerald-200 dark:bg-emerald-900/20 dark:text-emerald-400 dark:border-emerald-800/30' : 'bg-rose-50 text-rose-600 border-rose-200 dark:bg-rose-900/20 dark:text-rose-400 dark:border-rose-800/30' }}">
                                        @if($p->lp)<i class="fa-solid fa-check"></i> @else <i class="fa-solid fa-xmark"></i> @endif LP
                                    </span>
                                    <span class="inline-flex items-center gap-1.5 px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider border {{ $p->lpp ? 'bg-emerald-50 text-emerald-600 border-emerald-200 dark:bg-emerald-900/20 dark:text-emerald-400 dark:border-emerald-800/30' : 'bg-rose-50 text-rose-600 border-rose-200 dark:bg-rose-900/20 dark:text-rose-400 dark:border-rose-800/30' }}">
                                        @if($p->lpp)<i class="fa-solid fa-check"></i> @else <i class="fa-solid fa-xmark"></i> @endif LPP
                                    </span>
                                    <span class="inline-flex items-center gap-1.5 px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider border {{ $p->skp ? 'bg-emerald-50 text-emerald-600 border-emerald-200 dark:bg-emerald-900/20 dark:text-emerald-400 dark:border-emerald-800/30' : 'bg-rose-50 text-rose-600 border-rose-200 dark:bg-rose-900/20 dark:text-rose-400 dark:border-rose-800/30' }}">
                                        @if($p->skp)<i class="fa-solid fa-check"></i> @else <i class="fa-solid fa-xmark"></i> @endif SKP
                                    </span>
                                </div>
                            </td>
                ...
                <table class="w-full text-sm text-left">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-surface-700">
                        ...
                        <tr x-show="search === '' || $el.dataset.search.includes(search.toLowerCase())" data-search="{{ strtolower(e($p->nama . ' ' . $p->nim . ' ' . $p->tempat_riset)) }}" class="hover:bg-emerald-50/30 dark:hover:bg-emerald-900/10 transition-colors group">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <div class="w-10 h-10 rounded-full bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 flex items-center justify-center shrink-0 border border-emerald-100 dark:border-emerald-800/30"><i class="fa-solid fa-user-graduate"></i></div>
                            ...
                            <td class="px-6 py-4 text-center">
                                @if($p->nilai > 0)
                                    <div class="inline-flex items-center gap-2">
                                        <div class="w-8 h-8 rounded-full bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 flex items-center justify-center"><i class="fa-solid fa-star text-xs"></i></div>
