...
class MahasiswaService
{
    ...
    protected function transformDatatableRows(Collection $users, int $start): array
    {
        ...
        foreach ($users as $index => $u) {
            ...
            $data[] = [
                ...
                'jenis_label' => $proposal ? $proposal->jns_pkl : '-',
                'jenis_class' => $proposal ? 'bg-blue-50 text-blue-700 dark:bg-blue-900/20 dark:text-blue-400' : 'bg-gray-50 text-gray-700 dark:bg-gray-900/20 dark:text-gray-400',
                'tempat' => $proposal ? e($proposal->tempat_riset) : '-',
                'mentor' => $proposal ? e($proposal->nama_mentor) : '-',
                'status_label' => $status['label'],
                'status_class' => $this->statusColorClasses($status['class']),
            ];
        }
    ...
    protected function statusColorClasses(string $class): string
    {
        return match ($class) {
